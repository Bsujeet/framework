...
function createRequest (reqBody, callback) {
    switch (reqBody.operation) {
        case 'CREATE': {
            ...
            // vm_name
            const paramVmName = reqBody.parameters.filter((param) => { return param.name === 'vmName'; });
            ...
            // OS
            const paramOs = reqBody.parameters.filter((param) => { return param.name === 'os'; });
            if (paramOs.length === 1) {
                workflow.parameters.filter((param) => { return param.name === 'os'; })[0].value = paramOs[0].value;
            } else {
